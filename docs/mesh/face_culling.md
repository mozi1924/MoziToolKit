...
- **典型方块**：台阶（Slab）、楼梯（Stairs）、活板门（Trapdoor）、地毯（Carpet）、雪层（Snow Layer）。
- **规则**：
  - 系统提取 3D 构件在 6 个外边界面的 2D 矩形投影（`FaceOcclusionRect`）。
  - 当下半台阶底面接触石头时，石头底面完整覆盖台阶底面，台阶底面自动剔除。
  - 两个同高度的下半台阶侧面相接时，侧面接触的下半部矩形区域 `[0, 0, 1, 0.5]` 被完全覆盖剔除。
  - 模型的非外接表面（`cullface == None`，如火把棍、花草十字交叉面、箱子盖等）永不剔除。
