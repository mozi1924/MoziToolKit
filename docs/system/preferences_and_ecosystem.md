...
### 1.2 全自由偏好设置面板 (`ui/preferences_menus.py`)
- **自由重排与自定义 Label**：用户可在偏好设置中对每个视图下的菜单项自由进行上移（`mozi.menu_move_item`）、下移、添加（`mozi.menu_add_item`）、移除（`mozi.menu_remove_item`）、启用/禁用（Toggle Enable），并能自定义菜单显示的别名（Label）。
- **配置导入/导出与重置**：支持将自定义菜单布局导出为 JSON 文件（`mozi.menu_export_config`）、从外部导入（`mozi.menu_import_config`）或一键恢复官方默认（`mozi.menu_reset_config`）。
...
### 2.1 架构与多后端解耦 (`utils/config/`)
- **`ConfigManager`** 单例门面（Facade）：维护全局 `_lock = threading.RLock()`，保证进程内所有 UI 属性变动、异步线程与后台任务对配置的读写绝对串行化。
- **插拔式存储后端 (`utils/config/backends/`)**：
  - **`JsonBackend` (`json_backend.py`)**：生产默认后端，持久化至 Blender 用户配置目录下的 `mozitoolkit_config.json`。
  - **`MemoryBackend` (`memory_backend.py`)**：全内存沙盒，用于单元测试与无头沙盒隔离。
  - **`BlenderBackend` (`blender_backend.py`)**：与 Blender 原生 `AddonPreferences` 属性组保持双向状态同步。
...
3. **结构校验与导入防护**：
   通过 `mozi.menu_import_config` 导入外部 JSON 时，必须先进行全量 Schema 校验（`ConfigData.from_dict`），只有在整个结构解析合法后方可应用，无效的外部 JSON 绝对不会污染已有配置。
...
### 5.2 自动化测试套件 (`tests/`)
- 在终端中通过 Blender 无头模式执行全量单元测试（54 个测试套件，470+ 单元测试）：
  ```bash
  blender --background --python-expr "import unittest, sys; sys.path.insert(0, '/path/to/parent'); import MoziToolKit; suite = unittest.TestLoader().discover('MoziToolKit/tests', pattern='test_*.py'); unittest.TextTestRunner(verbosity=2).run(suite)"
  ```
- 重点测试覆盖：
  - `test_adaptive_pixel_split.py`：自适应网格细分与权重迁移测试。
  - `test_atlas_addressing.py`：图集坐标寻址与安全 Clamp 测试。
  - `test_extrude_repair_atlas.py`：侧面 UV 塌陷修复与模式判定测试。
  - `test_mc_model_baker.py` / `test_json_first_baker.py`：BlockState 变体、Multipart 条件与模型继承树展开测试。
  - `test_config_manager.py` / `test_sandbox_isolation.py`：配置原子写入、备份恢复、JSON Schema 校验与沙盒隔离测试。
  - `test_live_sync_protocol_and_storage.py` / `test_direct_mesh_sync.py`：实时同步二进制协议与网格生成测试。
  - `test_pbr_pack_stack.py` / `test_data_driven_atlas_system.py`：PBR 伴生贴图分层解析与图集装箱测试。
