...
def subdivide_quad_face(
    ...
    cols: Union[int, Sequence[float]],
    rows: Union[int, Sequence[float]],
    normalize_uvs: bool = False,
    uv_layer: Optional[bmesh.types.BMLoopUV] = None,
) -> List[bmesh.types.BMFace]:
    """Subdivide a single Quad face into a grid of quad sub-faces.

    Supports uniform integer divisions (cols x rows) or non-uniform fractional parameter cut lists
    [0.0, ..., 1.0] snapping strictly to integer pixel grid lines.
    Migrates vertex deform weights (armatures/skinning), face/vert/edge/loop attributes,
    UV seams, creases, and sharpness. Cleans up original face and orphan edges.

    :param bm: BMesh object
    :param face: Target Quad face to subdivide
    :param cols: Number of horizontal subdivisions (int >= 1) or sequence of U cut factors [0.0..1.0]
    :param rows: Number of vertical subdivisions (int >= 1) or sequence of V cut factors [0.0..1.0]
    :param normalize_uvs: If True, assign [0, 1] local UV coordinates to each sub-quad;
                          if False, bilinearly interpolate original UV coordinates.
    ...
    # Normalize cut factor arrays
    if isinstance(cols, (list, tuple)):
        u_cuts = list(cols)
        col_count = max(1, len(u_cuts) - 1)
    else:
        col_count = max(1, int(cols))
        u_cuts = [c / col_count for c in range(col_count + 1)]

    if isinstance(rows, (list, tuple)):
        v_cuts = list(rows)
        row_count = max(1, len(v_cuts) - 1)
    else:
        row_count = max(1, int(rows))
        v_cuts = [r / row_count for r in range(row_count + 1)]
    ...
    # 0. Extract all face custom attributes
    face_layer_values = _extract_all_face_layers(bm, face, is_subdivided=(col_count > 1 or row_count > 1))
    ...
    # 2D Grid of vertices: shape (row_count + 1) x (col_count + 1)
    grid_verts = [[None for _ in range(col_count + 1)] for _ in range(row_count + 1)]

    # Populate corner vertices to preserve topology references
    grid_verts[0][0] = v0
    grid_verts[0][col_count] = v1
    grid_verts[row_count][col_count] = v2
    grid_verts[row_count][0] = v3
    ...
    for r in range(row_count):
        for c in range(col_count):
            ...
            if r == row_count - 1:
                e_top = _get_edge_between(bm, grid_verts[row_count][c], grid_verts[row_count][c + 1])
                if e_top:
            ...
            if c == col_count - 1:
                e_right = _get_edge_between(bm, grid_verts[r][col_count], grid_verts[r + 1][col_count])
                if e_right:
