...
# perceptual roughness fix, non-negative SSS clamping, and Disable Subsurface toggle.
LABPBR_REFERENCE_LAYOUT_NODE_COUNT = 94
LABPBR_REFERENCE_LAYOUT_LINK_COUNT = 136
LABPBR_REFERENCE_NODE_COUNT = 61
LABPBR_REFERENCE_LINK_COUNT = 101
...
LABPBR_INTERFACE = (
    ...
    ("Disable Subsurface", "INPUT", "NodeSocketBool"),
)
...
def ensure_labpbr_decoder() -> bpy.types.NodeTree:
    ...
    has_color_data = _math(nodes, "Has Color Data", "GREATER_THAN", (-800, 820), {"Value[1]": 0.001})
    clean_albedo = node(nodes, "ShaderNodeMix", "Clean Albedo Color", location=(-600, 950), properties={"data_type": "RGBA", "blend_type": "MIX"}, inputs={"A[2]": (1.0, 1.0, 1.0, 1.0)})
    albedo_ao = node(nodes, "ShaderNodeMixRGB", "Albedo × Material AO", location=(-350, 820), properties={"blend_type": "MULTIPLY"}, inputs={"Factor": 1.0})
    ...
    enable_ao = node(nodes, "ShaderNodeMix", "Enable AO", location=(-100, 820), properties={"data_type": "RGBA", "blend_type": "MIX"})
    ...
    final_sss_weight = node(nodes, "ShaderNodeMix", "Filter Subsurface Weight", location=(0, -1020), properties={"data_type": "FLOAT", "blend_type": "MIX"}, inputs={"B[0]": 0.0})
    ...
    cutout_alpha = _math(nodes, "Cutout Alpha", "MULTIPLY", (650, 200))
    final_alpha = node(nodes, "ShaderNodeMix", "Final Alpha", location=(850, 200), properties={"data_type": "FLOAT", "blend_type": "MIX"}, inputs={"B[0]": 1.0})

    for child in (decode_normal, has_color_data, clean_albedo, albedo_ao, normal_x, normal_y, x_squared, y_squared, xy_squared, normal_z_base, clamp_normal_z, normal_z, encode_x, encode_y, encode_z, reconstructed_normal, normal_map, height_bump, height_minus_one, labpbr_depth, effective_displacement):
    ...
    link(links, group_input, "Albedo Alpha", has_color_data, "Value[0]")
    link(links, has_color_data, "Value", clean_albedo, "Factor[0]")
    link(links, group_input, "Albedo Color", clean_albedo, "B[2]")

    link(links, group_input, "Normal (_n) Color", decode_normal, "Color")
    link(links, decode_normal, "Blue", albedo_ao, "Color2")
    link(links, clean_albedo, "Result[2]", albedo_ao, "Color1")
    ...
    link(links, enable_labpbr, "Value", enable_ao, "Factor[0]"); link(links, clean_albedo, "Result[2]", enable_ao, "A[2]"); link(links, albedo_ao, "Color", enable_ao, "B[2]")
    ...
    link(links, group_input, "Disable Subsurface", final_sss_weight, "Factor[0]")
    link(links, enable_sss, "Value", final_sss_weight, "A[0]")
    ...
    link(links, group_input, "Albedo Alpha", cutout_alpha, "Value[0]")
    link(links, has_alpha, "Value", cutout_alpha, "Value[1]")
    link(links, group_input, "Transmission Weight", final_alpha, "Factor[0]")
    link(links, cutout_alpha, "Value", final_alpha, "A[0]")
    ...
    link(links, final_alpha, "Result[0]", principled, "Alpha")
