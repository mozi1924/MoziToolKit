...
class BlockStateResolver:
    ...
    def resolve_state(self, state_str: str) -> list[VariantMatch]:
        ...
        if not raw_state:
            ...
            if namespace == "minecraft" and short_name == "bell":
            ...
            if namespace == "minecraft" and "glazed_terracotta" in short_name:
                rot_y = {"south": 0.0, "west": 90.0, "north": 180.0, "east": 270.0}.get(facing, 0.0)
            elif namespace == "minecraft" and short_name in ("dispenser", "dropper"):
                if facing == "up":
            ...
            elif namespace == "minecraft" and short_name == "barrel":
                ...
                if facing == "up":
                    ...
                    rot_x = 90.0
            return [VariantMatch(model_id=f"{namespace}:block/{short_name}", rot_x=rot_x, rot_y=rot_y)]

        # 1. Variants format
        if "variants" in raw_state:
            variants = raw_state["variants"]
            match = self._match_variant(variants, props, default_namespace=namespace)
            if match:
                return [match]
            # Fallback to first variant if no exact match
            if "" in variants:
                return [self._parse_variant_entry(variants[""], default_namespace=namespace)]
            elif variants:
                first_key = next(iter(variants))
                return [self._parse_variant_entry(variants[first_key], default_namespace=namespace)]
        ...
        if "multipart" in raw_state:
            ...
            for part in raw_state["multipart"]:
                ...
                if when is None or self._evaluate_multipart_when(when, props):
                    if isinstance(apply, list):
                        ...
                        results.append(self._parse_variant_entry(apply[0], default_namespace=namespace))
                    else:
                        results.append(self._parse_variant_entry(apply, default_namespace=namespace))
            if results:
                return results
            ...
            for part in raw_state["multipart"]:
                ...
                if apply:
                    if isinstance(apply, list):
                        return [self._parse_variant_entry(apply[0], default_namespace=namespace)]
                    else:
                        return [self._parse_variant_entry(apply, default_namespace=namespace)]

        return [VariantMatch(model_id=f"{namespace}:block/{short_name}")]

    def _match_variant(
        self, variants: dict[str, Any], props: dict[str, str], default_namespace: str = "minecraft"
    ) -> Optional[VariantMatch]:
        # Form exact sorted key
        exact_key = ",".join(f"{k}={v}" for k, v in sorted(props.items()))
        if exact_key in variants:
            return self._parse_variant_entry(variants[exact_key], default_namespace=default_namespace)
        ...
        for v_key, v_entry in variants.items():
            if not v_key:
                if not props and best_score < 0:
                    best_score = 0
                    best_match = self._parse_variant_entry(v_entry, default_namespace=default_namespace)
                continue
            ...
            if score > best_score:
                best_score = score
                best_match = self._parse_variant_entry(v_entry, variant_props=v_props, default_namespace=default_namespace)

        return best_match

    def _parse_variant_entry(
        self, entry: Any, variant_props: Optional[dict[str, str]] = None, default_namespace: str = "minecraft"
    ) -> VariantMatch:
        ...
        if model_id and ":" not in model_id:
            model_id = f"{default_namespace}:{model_id}"

        if isinstance(entry, str):
            return VariantMatch(model_id=model_id, variant_props=variant_props)
        return VariantMatch(
            model_id=model_id,
