"""
Authoritative Built-in Block Entity OBJ Model Presets for Minecraft Vanilla Blocks.
Provides pixel-perfect models from jmc2obj (GPL v2) and authoritative specifications
for complex entity blocks: Chests, Shulker Boxes, End Portal Frame, End Portal,
End Gateway, Banners, Heads/Skulls, Bells, Decorated Pots, Conduits, and Hanging Signs.
"""
...
class BuiltinOBJCache:
    """In-memory cache for parsed built-in OBJ preset files."""
    def __init__(self):
        self._cache: dict[str, list[OBJRawFace]] = {}

    def load(self, model_filename: str) -> list[OBJRawFace]:
        ...
        faces = WavefrontOBJParser.parse_file(filepath)
        self._cache[model_filename] = faces
        return faces


_BUILTIN_CACHE = BuiltinOBJCache()
...
def build_baked_model_from_obj(
    ...
    """
    faces_list = _BUILTIN_CACHE.load(obj_filename)
    if not faces_list:
        return None
    ...
    if sub_objects:
        if isinstance(sub_objects, str):
            ...
            allowed = set(sub_objects)
        faces_list = [f for f in faces_list if f.object_name in allowed]
    ...
    for f_data in faces_list:
        ...
        # 4. Map UVs: OBJ vt has v=1.0 at top. Store as (u, 1.0 - v_obj) so mesh_generator outputs exact UV
        mc_uvs = []
        for u, v in raw_uvs:
...
# --- Dedicated Preset Builders ---

def build_shulker_box_model(block_state: str, short_name: str, props: dict[str, str]) -> BakedModel:
    """Construct a pixel-perfect Shulker Box model from authoritative OBJ geometry."""
...
def build_conduit_model(block_state: str) -> BakedModel:
    """Construct Conduit 6x6x6 centered cube model."""
...
def build_end_portal_model(block_state: str) -> BakedModel:
    """Construct End Portal horizontal plane at Y=0.75."""
...
def build_end_gateway_model(block_state: str) -> BakedModel:
    """Construct End Gateway 1x1x1 solid block."""
    tex_id = "minecraft:entity/end_portal"
...
def build_bell_model(
    ...
    """Construct hybrid Minecraft Bell combining JSON support frame with author-crafted bell body."""
    ...
    body_elems = bell_body_model.elements if bell_body_model else []
    body_faces = bell_body_model.faces if bell_body_model else []

    combined_elements = list(support_elements or []) + list(body_elems)
    combined_faces = list(support_faces or []) + list(body_faces)
...
class SpecialModelRegistry:
    """Registry for specialized built-in Block Entity model builders."""
    def __init__(self):
    ...
    def resolve(self, block_id: str, props: dict[str, str], fallback_texture: str = "") -> Optional[BakedModel]:
        ...
        if namespace != "minecraft":
            # Built-in presets strictly only apply to vanilla minecraft blocks
            return None
...
def resolve_obj_model_for_state(
    ...
    """Primary dispatcher resolving known Block Entity special models."""
