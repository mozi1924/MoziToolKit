"""
Face effective texture resolution and adaptive grid analyzer.
"""
...
def get_face_effective_texture_info(
    ...
    img = find_albedo_image_from_material(mat) or find_face_image(face, obj, context)
    ...
    anim_info = get_material_animation_info(mat)
    if anim_info and anim_info["total_frames"] > 1:
        fw = int(anim_info["frame_width"])
        fh = int(anim_info["frame_height"])
        tf = int(anim_info["total_frames"])
        is_baked = False
        if uv_layer and face.loops:
            bounds = get_face_uv_bounds(face, uv_layer)
            ...
            if uv_node and "Atlas Mode" in uv_node.inputs and float(uv_node.inputs["Atlas Mode"].default_value) == 1.0:
                is_baked = True
...
def calculate_face_target_grid(
    ...
    if len(face.loops) == 4:
        ...
        # Calculate lengths of opposing edges in pixel space
