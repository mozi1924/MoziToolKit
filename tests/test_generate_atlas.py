...
class TestAtlasGenerator(unittest.TestCase):
    ...
    def test_atlas_keeps_the_largest_source_tile_resolution(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(len(mapping["chunks"]), 2)
            ...
            self.assertEqual(mapping["chunks"][0]["packing"], "grid")
            self.assertEqual(mapping["chunks"][1]["width"], 16)
            self.assertEqual(mapping["chunks"][1]["height"], 16)
            self.assertEqual(mapping["chunks"][1]["packing"], "rect_bin_pack")
            self.assertEqual(mapping["textures"]["large"]["chunk_id"], 0)
            self.assertEqual(mapping["textures"]["large"]["texture_id"], 1)
            self.assertEqual(mapping["textures"]["small"]["chunk_id"], 1)
            self.assertEqual(mapping["textures"]["small"]["texture_id"], 0)
            ...
            atlas_grid = Image.open(outputs["chunks"][0])
            self.assertEqual(atlas_grid.getpixel((0, 0)), (255, 0, 255, 255))
            self.assertEqual(atlas_grid.getpixel((32, 0)), (0, 255, 0, 255))
            atlas_rect = Image.open(outputs["chunks"][1])
            self.assertEqual(atlas_rect.getpixel((0, 0)), (255, 0, 0, 255))
